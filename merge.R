...
Histogram_PopOver18 <- ggplot(CombinedData,aes(x=popover18))+geom_histogram(bins=5,fill="green", color="black") +labs(x="Population Over 18") + ggtitle("Histogram for Population Over 18 years") #displayed a histogram using ggplot and obtimizing number of bins to 5 to resolve gaps in bins
Histogram_Rape <- ggplot(CombinedData,aes(x=Rape))+geom_histogram(bins=15,fill="yellow", color="black") +labs(x="Rape") + ggtitle("Histogram for Rape Rate") ##displayed a histogram using ggplot and obtimizing number of bins to 15 to resolve gaps in bins

#5)	Create a boxplot for the population, and a different boxplot for the murder rate.
Box_Pop <- ggplot(CombinedData,aes(x=factor(0),population)) + geom_boxplot() + labs(title="Box Plot for Population",x="Factors", y="Population") #displayed a box plot with factoring the data for x axis along with population on x axis
Box_Murder <- ggplot(CombinedData,aes(x=factor(0),Murder)) + geom_boxplot() + labs(title="Box Plot for Murder Rate",x="Factors", y="Murder Rate") #displayed a box plot with factoring the data for x axis along with Murder rate on x axis
...
Bar_Murder2<- ggplot(CombinedData, aes(x=statename, y=numMurders)) + geom_col() + ggtitle("Total Murders") + theme(axis.text.x = element_text(angle = 90, hjust = 1))  #adjusted the label tags on x axis by rotating them 90 degrees and setting horizontal adjustment to 1

#10) Generate a new bar chart, the same as in the previous step, but also sort the x-axis by the murder rate
Bar_Murder3 <- ggplot(CombinedData, aes(x=reorder(statename,Murder), y=numMurders, group = 1)) + geom_col() + ggtitle("Total Murders") + theme(axis.text.x = element_text(angle = 90, hjust = 1))  #used reorder function to sort statename by murder rate

#11)	 Generate a third bar chart, the same as the previous step, but also showing percentOver18 as the color of the bar
Bar_Murder4 <- ggplot(CombinedData, aes(x=reorder(statename,Murder), y=numMurders, fill=percentover18)) + geom_col() + ggtitle("Total Murders") + theme(axis.text.x = element_text(angle = 90, hjust = 1)) #used fill attribute to adjust the colors of the bar char by data of percentover18
...
Scatter_Pop <- ggplot(CombinedData,aes(x=population,y=percentover18)) + geom_point(aes(size=Murder,color=Murder)) #displayed scatter plot using geom-point() and used aes in that to denote the color and size of the scatter plot points to murder rate
